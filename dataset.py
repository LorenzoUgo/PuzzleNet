...
class FRDataset(torch.utils.data.Dataset):
    def __init__(
        self, path, train=True, split=0.8, config=None, file="buildings_f_train1024.npy"
    ):
        self.path = path
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(self.fpcs[index]).to(torch.float32), torch.from_numpy(
            self.rpcs[index]
        ).to(torch.float32)
...
def get_datasets():
    ...
    trainset = ModifiedTransformedDataset(
        traindataset, transforms.RandomTransformSE3(0.8)
    )
    testset = ModifiedTransformedDataset(
        testdataset, transforms.RandomTransformSE3(0.8)
    )
...
class bs_dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        if self.train:
            return (
                torch.from_numpy(self.up[index]).to(torch.float32),
...
class cad_dataset(torch.utils.data.Dataset):
    def __init__(
        self, path, train=True, split=0.8, config=None, name="np_oa_up_train.npy"
    ):
        self.path = path
    ...
    def __getitem__(self, index):
        return torch.from_numpy(self.up[index]).to(torch.float32), torch.from_numpy(
            self.down[index]
        ).to(torch.float32)
...
class cad_1_dataset(torch.utils.data.Dataset):
    ...
    def __init__(
        self, path, train=True, split=0.8, config=None, name="np_oa_all_train.npy"
    ) -> None:
        super().__init__()
        self.path = path
        self.all = np.load(os.path.join(self.path, name), allow_pickle=True)
    ...
    def __getitem__(self, index):
        ...
        self.up = self.fps(up, 1024)
        self.down = self.fps(down, 1024)
        return torch.from_numpy(self.up).to(torch.float32), torch.from_numpy(
            self.down
        ).to(torch.float32)
...
def get_cad_datasets(category="cad", random=False, random_slice=False):
    if category == "cad":
        ...
        trainset = MovedCADDataset(
            traindataset, transforms.RandomTransformSE3(0.8, True)
        )
        ...
        testset = MovedCADDataset2(
            testdatset, transforms.RandomTransformSE3(0.8, random)
        )
        return trainset, testset
...
def cylinder_split(points, z=None):
    cylinder = o3d.geometry.TriangleMesh.create_cylinder(
        radius=0.6, height=1, resolution=50
    )
    cylinder.rotate(
...
def get_test_dataset(category, random=False, random_slice=False):
    if category == "cadrr":
        ...
        tstdataset = MovedCADDataset2(
            dataset, transforms.RandomTransformSE3(0.8, random)
        )
        return tstdataset
    elif category == "cadrr2":
        dataset_path = "/home/code/transReg/data/cad"
        dataset = cad_3_dataset(
            dataset_path, split=1, name="np_oa_all_test.npy", random_add=random_slice
        )
        tstdataset = MovedCADDataset2(
            dataset, transforms.RandomTransformSE3(0.8, random)
        )
...
class CADDataset2(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if mode == "train":
            ...
            self.all = np.load(
                os.path.join(self.path, name.replace("_train", "_test")),
                allow_pickle=True,
            )
    ...
    def getitem_non_random(self, index):
        ...
        self.fpcb, self.rpcb, fpc_idx, rpc_idx = self.get_boundary(
            self.down, self.up
        )  # fpc boundary
        ...
        if slice_seed == 0:
            ...
            if uppc.shape[0] >= 1024 and downpc.shape[0] >= 1024:
                ...
                if re_now > 0.6:
                    ...
                    #  self.down = torch.cat([down, self.up], dim=0)
                    #  self.up = d
                    self.down, self.up = self.fps(self.down.numpy(), 1024), self.fps(
                        self.up.numpy(), 1024
                    )
                    self.up = torch.from_numpy(uppc).to(torch.float32)
                    self.down = torch.from_numpy(downpc).to(torch.float32)
        ...
        if type(item0) == list:
            ...
            for m in item0:
                mesh = o3d.io.read_triangle_mesh(
                    os.path.join("/home/code/transReg/data", m)
                )
            ...
            mesh = o3d.io.read_triangle_mesh(
                os.path.join("/home/code/transReg/data", item0)
            )
            pc = o3d.geometry.TriangleMesh.sample_points_uniformly(mesh, 2000)
            points1 = np.array(pc.points)
        mesh2 = o3d.io.read_triangle_mesh(
            os.path.join("/home/code/transReg/data", item1)
        )
...
class CADDataset(torch.utils.data.Dataset):
    ...
    def getitem_non_random(self, index):
        ...
        self.up = torch.from_numpy(self.up).float()
        self.down = torch.from_numpy(self.down).float()
        ...
        if slice_seed == 0:
            ...
            if uppc.shape[0] >= 1024 and downpc.shape[0] >= 1024:
                ...
                if se == 0 or down.shape[0] < 1024:
                    ...
                    up_down = self.fps(
                        np.vstack(([uppc, downpc][(choice + 1) % 2], down)), 1024
                    )
...
class BuildingDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        split=0.8,
        config=None,
        file="buildings_f_train1024.npy",
    ):
        self.path = path
        self.fpcs = np.load(os.path.join(self.path, file))
        self.rpcs = np.load(os.path.join(self.path, file.replace("_f_", "_r_")))

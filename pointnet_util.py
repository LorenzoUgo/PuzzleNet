...
def sample_and_group(npoint, radius, nsample, xyz, points, returnfps=False, knn=False):
    ...
    Input:
        ...
        points: input points data, [B, N, D], 特征
    Return:
    ...
    if knn:
        ...
        idx = dists.argsort()[:, :, :nsample]  # B x npoint x K new_xyz 在xyz点云里最近的nsample个点的索引
    else:
        idx = query_ball_point(radius, nsample, xyz, new_xyz)
    torch.cuda.empty_cache()
    grouped_xyz = index_points(xyz, idx) # [B, npoint, nsample, C]
    torch.cuda.empty_cache()
    grouped_xyz_norm = grouped_xyz - new_xyz.view(B, S, 1, C) # 归一化，减去了中心
    torch.cuda.empty_cache()

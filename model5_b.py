...
class Encoder(nn.Module):
    ...
    def forward(self, xyz):
        ...
        x2, f2 = self.sg2(
            256, 0, 32, x, f1f, False, True
        )  # [B, N, 3], [B, N, 32, 128+3] N = 256
        # print('aa', x2.shape)
        # print('aaa', f2.shape)
        ...
        attention = (
            attention / 4
        )  # 这是对x2这些点的点特征，一对多，每个点都对应了其他256个点的特征
...
class BiDecoderNoneCross(nn.Module):
    ...
    def forward(self, f_local, f_global):
        ...
        f_global = (
            f_global.unsqueeze(1) if len(f_global.shape) == 2 else f_global
        )  # [B, 1, 1024]
...
class PCTransformer_nonsort(nn.Module):
    ...
    def forward(self, xyz):
        ...
        x, f1 = self.sg1(
            512, 0, 32, xyz, x_feature, False, True
        )  # [B, N , 3] [B, N, 32, 64+3]
        # print(x.shape)
        # print(f1.shape)
        f1f = F.relu(self.mlp3(f1))
...
class TouchedRegraster(pl.LightningModule):
    def __init__(self, config):
        ...
        self.Encoder = PCTransformer_nonsort(
            config
        )  # TODO: 有个问题，现在的网络是否对旋转敏感？
        self.Encoder2 = PCTransformer_nonsort(
            config
        )  # TODO: 有个问题，现在的网络是否对旋转敏感？
        ...
        # self.Decoder2 = PCTransformerDecoder(config)
    ...
    def predict5(self, batch, batch_indic, need=False, training=False):
        ...
        #  import pdb
        #  pdb.set_trace()
        ffpc_feature4seg = torch.cat(
            [non_sg_fmrpc_global, non_sg_ffpc], dim=-1
        )  # [B, 1024, 128]
        fmrpc_feature4seg = torch.cat(
            [non_sg_ffpc_global, non_sg_fmrpc], dim=-1
        )  # [B, 1024, 128]
    ...
    def predict4(self, batch, batch_indic, need=False, training=False):
        ...
        if not training:
            ...
            self.Encoder.train()
            self.Decoder.train()
            print(" --- ---> Start mrpcbDecoder train <--- --- ")
            self.mrpcbDecoder.train()
            #  self.Encoder2.train()
            print(" --- ---> Start tfMLP train <--- --- ")
    ...
    def vis(self, tag, pc1, pc2, index, logger):
        ...
        color2 = torch.from_numpy(np.array([0, 0.651, 0.929]) * 255).expand(
            pc2.shape[1], 3
        )
        color = torch.cat([color1, color2], axis=0)
        logger.experiment.add_mesh(
            tag, pc.unsqueeze(0), color.unsqueeze(0), global_step=self.global_step
        )
    ...
    def training_step(self, batch, batch_indic):
        ...
        if pretrain:
            # 只训练mlp一个branch
            out, _, x2, attention, mrpc_x2, mrpc_attention = self.predict6(
                batch, batch_size, training=True, need=True, pretrain=True
            )
        elif not pretrain:
            out, t, x2, attention, mrpc_x2, mrpc_attention, de_fpcb, de_mrpcb = (
                self.predict5(batch, batch_size, training=True, need=True)
            )
        ...
        inverse_de_mrpcb = se3.transform(
            se3.exp(out).to(mrpc), de_mrpcb.permute(0, 2, 1)
        )
    ...
    def validation_step(self, batch, batch_size):
        ...
        if pretrain:
            # 只训练mlp一个branch
            out = self.predict6(
                batch, batch_size, training=False, need=False, pretrain=True
            )
        elif not pretrain:
            out, t, de_fpcb, de_mrpcb = self.predict5(
                batch, batch_size, training=False, need=False
            )
    ...
    def test_step(self, batch, batch_id):
        ...
        # 测配准部分的结果
        out, _, de_fpcb, de_mrpcb = self.predict5(
            new_batch, batch_size, training=False, need=False
        )
        #  import pdb
        #  pdb.set_trace()
    ...
    def test_epoch_end(self, outputs) -> None:
        ...
        with open(
            os.path.join(
                self.C.output_path,
                datetime.datetime.now().strftime("%b%d_%H-%M-%S") + "metrics.txt",
            ),
            "w+",
        ) as fout:
            fout.writelines(
                "r_mse,   r_mae,   t_mse,    t_mae,    r_iso,    t_iso,  fpc_iou,   mrpc_iou, cd_fpcb, cd_rpcb \n"
            )
            for ss in s:
    ...
    def vis_attention(self, tag, x2, attention, index, axis=True):
        ...
        the_fourth_dimension = (the_fourth_dimension - min(the_fourth_dimension)) / (
            max(the_fourth_dimension) - min(the_fourth_dimension)
        )
...
def parse_arg(argv=None):
    ...
    parser.add_argument(
        "--dt", help="data type of input, default=fr", type=str, default="fr"
    )
    parser.add_argument("--mode", help="mode for now", default="train")
    parser.add_argument("--batch_size", default=2, type=int)
    parser.add_argument(
        "--save_on_epochs", default=1, type=int, help="save on how many epochs"
    )
    parser.add_argument("--loss_mode", default=0, type=int, help="mode of loss using")
    parser.add_argument(
        "--valp", default=0, type=int, help="how often check valuation per epochs"
    )
    parser.add_argument(
        "-m",
        help="leave a message for this round of training/valuation",
        type=str,
    ...
    parser.add_argument(
        "--random", default=False, action="store_true", help="use random dataset"
    )
    parser.add_argument(
        "--use_emd2", action="store_true", help="look or not", default=False
    )
    parser.add_argument(
        "--use_cd2", action="store_true", help="look or not", default=False
    )
    parser.add_argument(
        "--use_emd3",
        action="store_true",
        help="use emd as loss for fpcb",
        default=False,
    )
    parser.add_argument(
        "--random_slice", action="store_true", help="look or not", default=False
    )
    parser.add_argument(
        "--verbose", action="store_true", help="verbose or not", default=False
    )
...
def main(opt):
    ...
    if opt.dataset == "cadrr":
        ...
        true_testdataloader = torch.utils.data.DataLoader(
            true_testdataset,
            batch_size=opt.batch_size,
            drop_last=False,
            shuffle=False,
        ...
        true_testdataset = dataset.get_test_dataset(
            opt.dataset, random=opt.random, random_slice=opt.random_slice
        )
        true_testdataloader = torch.utils.data.DataLoader(
            true_testdataset,
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        every_n_epochs=opt.save_on_epochs,
        save_top_k=3,
